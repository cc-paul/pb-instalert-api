<?php
    ...
    $stmt = $pdo->prepare("SELECT * FROM ar_otp WHERE otp = :otp AND email = :email AND isUsed = 0 LIMIT 1");
    ...
    if ($stmt->execute()) {
        ...
        if ($count == 0) {
            ...
            $stmt_insert = $pdo->prepare("UPDATE ar_otp SET isUsed = :isUsed WHERE otp = :otp");
            $stmt_insert->bindParam(":isUsed",$isUsed,PDO::PARAM_STR);
            $stmt_insert->bindParam(":otp",$otp,PDO::PARAM_STR);

            if ($stmt_insert->execute()) {
                
                $stmt_insert = $pdo->prepare("UPDATE ar_user_registation SET `password` = MD5(:password) WHERE emailAddress = :email");
